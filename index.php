...
if ( strpos( $_SERVER[ 'SERVER_NAME' ], 'localhost' ) !== false ) {
	$f3->set( 'url', 'http://localhost/github/AMP-btc.animtype/' );
} else {
    if ($f3->get('SCHEME')!='https')  {
    	$f3->reroute('https://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']);
	}
	$f3->set( 'url', 'https://btc.animtype.com/' );
}
...
$f3->route('GET /legal-notice','WebPage->ampLegalPage',0);
$f3->route('GET /*','WebPage->ampPage',0);
